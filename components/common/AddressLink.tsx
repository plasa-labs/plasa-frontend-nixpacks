import { ExternalLink } from 'lucide-react'

/**
 * Props interface for the AddressLink component
 * @interface AddressLinkProps
 * @property {string} address - The blockchain address to display and link to
 * @property {string} [className] - Optional CSS classes to apply to the link
 */
interface AddressLinkProps {
	address: string
	className?: string
}

/**
 * AddressLink component displays a blockchain address in abbreviated form with an external link
 * @param {AddressLinkProps} props - Component props
 * @returns {JSX.Element} Rendered address link with external link icon
 */
export default function AddressLink({ address, className = '' }: AddressLinkProps) {
	const shortAddress = `${address.slice(0, 6)}...${address.slice(-4)}`
	const explorerUrl = `https://sepolia.basescan.org/address/${address}`

	return (
		<a
			href={explorerUrl}
			target="_blank"
			rel="noopener noreferrer"
			className={`inline-flex items-center gap-1 text-sm hover:underline ${className}`}
		>
			{shortAddress}
			<ExternalLink className="h-3 w-3" />
		</a>
	)
}
