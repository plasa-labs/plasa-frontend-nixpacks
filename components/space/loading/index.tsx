/**
 * Barrel file for Space-related loading/skeleton components
 * These components are used as placeholders while the actual content is being loaded
 * to improve perceived performance and user experience
 */

export { default as SpaceHeaderSkeleton } from './SpaceHeaderSkeleton'
export { default as SpaceQuestionsListSkeleton } from './SpaceQuestionsListSkeleton'
export { default as SpaceLeaderboardSkeleton } from './SpaceLeaderboardSkeleton'
export { default as SpaceInformationSkeleton } from './SpaceInformationSkeleton'

